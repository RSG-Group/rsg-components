...
export default class RSGCheckbox extends React.Component {
  constructor(props) {
    super(props);
  ...
  render(){
    ...
    return (
      <span>
        <style>{styles}</style>
        <label className={classy} style={this.props.style} {...omit(this.props, ["className", "onChange"])}>
          <input type="checkbox"
            onChange={this.handleChange.bind(this)} 
            checked={this.state.checked}
            key={1} {...omit(this.props, ["type", "onChange"])}
          />
          <span key={2} />
        </label>
      </span>
